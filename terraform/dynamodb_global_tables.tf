...
# Controlled by enable_global_tables variable (default: false)

# Global Tables provide:
# - Multi-region replication (Primary: us-east-1, DR: us-east-2)
...
resource "aws_dynamodb_table" "users_replica" {
  count          = var.enable_global_tables && var.enable_dr ? 1 : 0
  provider       = aws.dr
...
resource "aws_cloudwatch_metric_alarm" "dr_users_table_throttles" {
  ...
  dimensions = {
    TableName = aws_dynamodb_table.users_replica[0].name
  }
}
...
output "dynamodb_dr_users_table_name" {
  ...
  value       = var.enable_global_tables && var.enable_dr ? aws_dynamodb_table.users_replica[0].name : "DR disabled"
}

output "dynamodb_dr_threads_table_name" {
  description = "Name of the DR Threads DynamoDB table"
  value       = var.enable_global_tables && var.enable_dr ? aws_dynamodb_table.threads_replica[0].name : "DR disabled"
}

output "dynamodb_dr_posts_table_name" {
  description = "Name of the DR Posts DynamoDB table"
  value       = var.enable_global_tables && var.enable_dr ? aws_dynamodb_table.posts_replica[0].name : "DR disabled"
}

output "global_tables_enabled" {
  description = "Whether Global Tables are enabled"
  value       = var.enable_global_tables && var.enable_dr
}
